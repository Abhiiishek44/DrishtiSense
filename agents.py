...
SUBSCRIPTION GRAPH (enforced by register()):
  ...
  navigation/route_rejectedural design)

WHAT CHANGED FROM v4 → v5
──────────────────────────
v4 agents:
  - __init__(self, ..., emit: LogEmitter, ...)
  - expose direct-call methods: process()ed   → CoordinatorAgent.on_route_rejected
...
_OBJECT_SYNONYMS = {
    ...
    "eyeglasses": "eyeglasses", "eye glasses": "eyeglasses",
    "glasses": "eyeglasses", "spectacles": "eyeglasses",
}
_STOP_WORDS = {
...
def _deterministic_parse(text: str) -> dict:
    ...
    # Prefer the longest whole phrase. Substring matching used to interpret
    # "eye glasses" as "glass" → "wine glass", sending the focused detector,
    # radar, and memory down the wrong class path.
    for syn, label in sorted(_OBJECT_SYNONYMS.items(), key=lambda item: len(item[0]),
                             reverse=True):
...
class LibrarianAgent:
    ...
    def __init__(self, bus: EventBus, db, live_candidate_provider=None):
        self._bus = bus
        self._db = db
        self._live_candidate_provider = live_candidate_provider
    ...
    async def on_query_received(self, event: Event) -> None:
        ...
        # Let the focused live scan collect several tracker matches before
        # falling back to a remembered location.  This is bounded so queries
        # remain responsive even if the target is not in view.
        if self._live_candidate_provider:
            await asyncio.sleep(0.6)
        # Live, verified tracks always precede a database result.  The
        # provider is owned by the camera pipeline; the Librarian only asks
        # for candidates and keeps the source ranking explicit.
        live_results = self._live_candidate_provider(target) if self._live_candidate_provider else []
        # A verified live target is authoritative. Do not wait for Qdrant or
        # let an old memory delay/override current camera evidence.
        if live_results:
            results = list(live_results)
            await _bus_emit(self._bus, AgentLog(
                agent=self.NAME, level="success",
                message=f"Using verified live target '{target}'; memory lookup skipped.",
            ))
        else:
        ...
        try:
            raw_results = await asyncio.wait_for(
                asyncio.get_running_loop().run_in_executor(
                    None, lambda: self._db.search(target, min_confidence=min_confidence)
                ),
                timeout=2.0,
            )
        except asyncio.TimeoutError:
            log.warning("Memory search timed out for '%s'", target)
            await _bus_emit(self._bus, AgentLog(
                agent=self.NAME, level="warning",
                message=f"Memory search timed out; continuing live search for '{target}'.",
            ))
            return []
        except Exception as exc:
            # EventBus isolation prevents a DB exception from crashing other
            # agents, but swallowing it here used to leave the UI waiting
            # forever. Treat unavailable memory as empty and let live search
            # continue independently.
            log.warning("Memory search unavailable for '%s': %s", target, exc)
            await _bus_emit(self._bus, AgentLog(
                agent=self.NAME, level="warning",
                message=f"Memory backend unavailable; continuing live search for '{target}'.",
            ))
            return []
...
class CoordinatorAgent:
    ...
    def __init__(self, bus: EventBus, llm, world_model: WorldModel):
        ...
        self._llm = llm
        self._world = world_model
        # The compass is driven by the fast vision loop.  Keep its latest
        # absolute heading here so asynchronous navigation queries are based
        # on the user's current orientation.
    ...
    def update_heading(self, heading: float) -> None:
        """Receive the latest absolute compass heading from the vision loop."""
    ...
    async def on_search_result(self, event: Event) -> None:
        ...
        # An old observation is useful context, but it is not a live route.
        # Never instruct a user to navigate to it as though it were current.
        if best.source == "memory" and best.effective_confidence < 0.35:
            await self._bus.publish(
                "navigation/route_final",
                RouteFinalPayload(
                    response_text=(f"I can't see the {best.memory.label} now. "
                                   f"I last saw it {format_time_ago(best.age_seconds)} ago."),
                    spatial=None, verdict=None, query_text=payload.query_text,
                ), publisher=self.NAME,
            )
            return
    ...
    async def compose_response(
        ...
        # Direction and distance are safety-critical, so user-facing navigation
        # is composed only from the validated SpatialResult. The LLM may parse
        # an ambiguous target, but it never writes geometry or movement advice.
        response = spatial.to_speech()
        await _bus_emit(self._bus, AgentLog(
            agent=self.NAME, level="success",
            message=f'Deterministic response: "{response}"',
        ))
        return response
...
class CriticAgent:
    ...
    async def _validate(
        ...
        # Multi-frame live semantic detections use a lower calibrated floor
        # than decayed memories; they have independent temporal validation.
        confidence_threshold = self._threshold
        if results and results[0].source in {"live", "tracked"} and \
                results[0].consecutive_frames >= 3:
            confidence_threshold = 0.30
        # Rule 1: Confidence threshold (hard)
        if spatial.confidence < confidence_threshold:
            approved = False
            reasons.append(
                f"Confidence {spatial.confidence:.0%} < threshold {confidence_threshold:.0%}"
            )

        # A semantic candidate is not a route until the tracker has observed
        # it consistently. This duplicates the vision gate intentionally: the
        # Critic remains the final safety boundary for agent guidance.
        if results and results[0].source in {"live", "tracked"} and \
                results[0].consecutive_frames < 3:
            approved = False
            reasons.append(
                f"Tracking stability {results[0].consecutive_frames} frames < required 3"
            )
        ...
        if spatial.distance_m <= 0.10:
            approved = False
            reasons.append("Distance is not physically reliable")
        elif spatial.distance_m > 8.0:
