...
class Settings(BaseSettings):
    ...
    LLM_PROVIDER: str = "auto"  # "huggingface" to prefer HF, otherwise "auto"
    HF_TOKEN: str = ""
    HF_MODEL: str = "deepseek-ai/DeepSeek-R1:fastest"
    HF_MAX_TIMEOUT_MS: int = 15000
    ...
    # Open-vocabulary classes (for example, "shirt") are less certain than
    # COCO classes. Use a more sensitive, higher-resolution pass for them.
    OPEN_VOCAB_CONFIDENCE: float = 0.20
    OPEN_VOCAB_IMAGE_SIZE: int = 960
    # A broad home scan has many competing prompts and therefore needs a
    # stricter cutoff than a focused scan for one requested object.
    HOME_SCAN_CONFIDENCE: float = 0.35
    # Query-triggered hybrid detector. Models are lazy-loaded on the first
    # focused search so normal startup and continuous YOLO remain lightweight.
    HYBRID_VISION_ENABLED: bool = True
    GROUNDING_DINO_MODEL: str = "IDEA-Research/grounding-dino-tiny"
    GROUNDING_DINO_BOX_THRESHOLD: float = 0.22
    GROUNDING_DINO_TEXT_THRESHOLD: float = 0.20
    SAM2_MODEL: str = "sam2.1_t.pt"
    FOCUSED_REACQUIRE_SECONDS: float = 1.5
    VISION_FPS: int = 8
    ...
    # JSON-backed metric world positions, retained after tracks leave view.
    WORLD_MEMORY_PATH: str = "lumina_world_memory.json"
    # Close enough to reach while avoiding premature "arrived" calls from a
    # coarse monocular depth estimate.
    GOAL_ARRIVAL_DISTANCE_M: float = 0.22
...
class LLMClient:
    ...
    def __init__(self, groq_key: str, openai_key: str,
                 ...
                 hf_token: str = "", hf_model: str = "", hf_timeout_ms: int = 15000,
        ...
        self.hf_timeout_s = max(1.0, hf_timeout_ms / 1000)
    ...
    async def _hf_complete(self, system: str, user: str,
                           max_tokens: int, temperature: float) -> Optional[str]:
        """Call Hugging Face Inference Providers through its chat API."""
        if not self.hf_token or not self.hf_model:
            return None
        try:
            import httpx
            async with httpx.AsyncClient(timeout=self.hf_timeout_s) as client:
    ...
    async def complete(self, system: str, user: str,
        ...
        # ── 1. Hugging Face (when explicitly selected) ─────────
        if self.preferred_provider == "huggingface":
            t0 = _time.perf_counter()
            text = await self._hf_complete(system, user, max_tokens, temperature)
            if text:
                log.info(f"[Hugging Face] {(_time.perf_counter()-t0)*1000:.0f}ms — {len(text)} chars")
                self.active_model = f"huggingface/{self.hf_model}"
                return text
        ...
        # ── 5. Deterministic fallback (never fatal) ────────────
    ...
    async def health_check(self) -> dict:
        results = {
            "groq": False,
            "openai": False,
            "huggingface": bool(self.hf_token and self.hf_model),
...
class SpatialDatabase:
    ...
    def _semantic_search(self, query: str, session_id: str, top_k: int,
        ...
        try:
            ...
            # qdrant-client 1.10+ replaced `.search()` with
            # `.query_points()` and returns hits under `.points`.
            if hasattr(self._client, "query_points"):
                response = self._client.query_points(
                    collection_name=self.col_obj, query=vector,
...
@app.get("/camera.jpg")
def camera_jpeg():
    """Return one camera frame for the built-in preview page.

    This runs in FastAPI's worker pool, so a blocking webcam read cannot
    starve the async WebSocket/event-bus loop.
    """
    orchestrator = app.state.orchestrator
    if not orchestrator._camera:
        return Response(status_code=503, content="Camera is still starting")
    # The camera capture worker is the only V4L2 reader. Preview and vision
    # both use its cached frame so neither can starve the other.
    frame = orchestrator._camera.latest_frame()
    if frame is None:
        return Response(status_code=503, content="Camera returned no frame")
    from vision import frame_to_b64
    import base64
    return Response(
        content=base64.b64decode(frame_to_b64(frame, settings.FRAME_JPEG_QUALITY)),
...
@app.post("/detect-now")
async def detect_now(payload: dict):
    """Run a requested detection immediately and persist any result."""
    classes = payload.get("classes", [])
    try:
        detections = await app.state.orchestrator.detect_once(classes)
        return JSONResponse({"detections": detections})
    except Exception as e:
        _api_log.exception("Dashboard detection failed")
...
@app.get("/find-object/{label}")
async def find_object(label: str):
    """Locate a remembered object relative to the current camera pose."""
    orchestrator = app.state.orchestrator
    result = orchestrator.find_object(label)
    if result is None:
        return JSONResponse({"error": f"No remembered '{label}'"}, status_code=404)
    return JSONResponse(result)


@app.get("/world-memory")
async def get_world_memory():
    """Return persistent objects relative to the latest camera pose."""
    return JSONResponse({"objects": app.state.orchestrator.get_world_memory()})


@app.get("/goal")
async def get_navigation_goal():
    return JSONResponse(app.state.orchestrator.get_goal())


@app.post("/goal/{label}")
async def start_navigation_goal(label: str):
    """Start deterministic guidance toward a visible or remembered object."""
    event = app.state.orchestrator.start_goal(label)
    await app.state.orchestrator._broadcast_goal(event)
    status_code = 404 if event["status"] == "not_found" else 200
    return JSONResponse(event, status_code=status_code)


@app.delete("/goal")
async def cancel_navigation_goal():
    event = app.state.orchestrator.cancel_goal()
    await app.state.orchestrator._broadcast_goal(event)
    return JSONResponse(event)


@app.get("/safe-path")
async def get_safe_path():
    """Return the current three-corridor safe-path visualization estimate."""
    return JSONResponse(app.state.orchestrator.get_safe_path())


@app.post("/camera-pose")
async def update_camera_pose(payload: dict):
    """Accept metric camera poses from an ARCore or VIO mobile client."""
    try:
        result = await app.state.orchestrator.update_camera_pose(
            x=payload["x"], y=payload.get("y", 0.0), z=payload["z"],
            yaw_deg=payload["yaw_deg"], source=payload.get("source", "arcore"),
        )
    except (KeyError, TypeError, ValueError) as exc:
        return JSONResponse({"error": f"Invalid camera pose: {exc}"}, status_code=422)
    return JSONResponse(result)
...
async def get_status():
    ...
    return JSONResponse({
        ...
        "huggingface": llm_health.get("huggingface", False),
        ...
        "persistent_world_memory": orchestrator._persistent_memory.summary(),
        "camera_pose_source": orchestrator._pose_tracker.pose.source,
        "focused_vision": (orchestrator._focused_detector.status()
                           if orchestrator._focused_detector else {"configured": False}),
    })
