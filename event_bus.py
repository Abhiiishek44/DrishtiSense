...
# User interaction has a separate low-latency lane. These events do not outrank
# hardware safety; they simply avoid waiting behind continuous frame-derived
# memory writes. Each handler still has the same exception isolation.
USER_RESPONSE_TOPICS: Set[str] = {
...
class EventBus:
    ...
    async def publish(self, topic: str, payload: Any, publisher: str = "UNKNOWN") -> Event:
        ...
        if topic in HIGH_PRIORITY_TOPICS or topic in USER_RESPONSE_TOPICS:
...
class FramePayload:
    ...
    heading: float
    frame_id: int
    # Confirmed tracks are the contract consumed by ArchivistAgent to create
    # persistent spatial memories. Keeping a default preserves compatibility
    # for producers that only publish raw frames.
