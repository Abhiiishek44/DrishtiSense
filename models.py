...
KNOWN_HEIGHTS_M: dict[str, float] = {
    ...
    "book": 0.28, "picture frame": 0.35, "backpack": 0.50,
...
class MemorySearchResult(BaseModel):
    ...
    # The retrieval policy is live-first.  Keeping the source on the result
    # lets the Coordinator and Critic distinguish a current observation from
    # a remembered location without changing the spatial geometry contract.
    source: Literal["live", "tracked", "memory"] = "memory"
...
class SpatialResult:
    ...
    def to_speech(self) -> str:
        ...
        estimate = " This location may be out of date." if self.confidence < 0.45 else ""
        return (
            f"{stale_prefix}{self.turn_instruction}. Your {self.label} is "
            f"{self.distance_str} away at {self.clock_direction}.{estimate}"
        )
